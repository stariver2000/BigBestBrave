 ...
 *
 * 가져오지 않은 것
 *   - ML 모델과 SHAP 계산. 이 사이트는 연산 예산이 0이고, 모델 없이 흉내 내면 지어낸
 *     예측이 된다. 그래서 이 화면의 개입은 예측이 아니라 시연이다 - 단추를 눌러야 뜬다.
...
/** 시연에서 설명으로 보여 줄 세 갈래. 표 2의 다섯 갈래 가운데 셋을 고르는 것은 원래 모델의 몫이었다. */
export const DEMO_CATEGORIES = ['Phone & App Use', 'Location', 'Time'] as const;
